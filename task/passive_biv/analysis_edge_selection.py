...
from task.passive_biv.data_preparation.datasets import FEHeartSageDataset
...
class FEHeartSageV2Evaluation(FEHeartSageDataset):
    ...
    def single_graph_evaluation(self):
        ...
        inputs, actual_output = transform(data)
        ...
        with torch.no_grad():
            ...
            eval_metric = EuclideanDistanceMSE().forward(output["displacement"], actual_output["displacement"])
    ...
    def _init_transform(self):
        ...
        transform_list.append(ConvertToModelInputsWithSelectedNode(convert_model_input_config, True))
...
if __name__ == "__main__":
    ...
    config.task_data["sections"] = [0, 20, 100]
    config.task_data["nodes_per_sections"] = [6, 6]

    eval_metrics_loss = []
    total_idx = 20
    for i in range(total_idx):
        evaluation = FEHeartSageV2Evaluation(config.task_data, "eval", i)

        eval_metrics = evaluation.single_graph_evaluation()

        eval_metrics_loss.append(eval_metrics.item())

        print(i, eval_metrics.item())

    print(eval_metrics_loss, np.sum(eval_metrics_loss) / total_idx)
