...
from pkg.train.datasets.base_datasets_train import BaseIterableDataset, IndexedTrainDataset
...
from pkg.utils.other_utils.io import load_yaml
...
class TrainerConfig(IConfig):
    ...
    def __init__(self) -> None:
        ...
        self.task_base["gpu_num"] = self.task_base.get("gpu_num", None)
        ...
        self.task_data["base_data_path"] = self.task_data.get(
            "base_data_path", f"{repo_path}/data/{self.task_data['dataset_name']}"
        )

        # task trainer
        self.task_trainer = config["task_trainer"]
        self.task_trainer["device"] = "cuda" if self.task_base["gpu"] else "cpu"
        ...
        self.task_train["device"] = "cuda" if self.task_base["gpu"] else "cpu"
...
class BaseTrainer(abc.ABC):
    ...
    def create_dataset(
        ...
        train_dataset = self.dataset_class(train_data_config)
        logger.info(f"Number of train data points: {len(train_dataset)}")

        val_data_config = {**task_data, **{"data_type": VALIDATION_NAME}}
        validation_dataset = self.dataset_class(val_data_config)
    ...
    def print_model(self, model: nn.Module, inputs: Dict):
        ...
        if self.gpu:
            logger.info("when using cuda, we do not support model summary function")
            return
    ...
    def create_lr(self) -> None:
        ...
        if scheduler_method == "multi_step":
            batch_per_epoch = optimizer_param["batch_per_epoch"]
            milestones = optimizer_param["milestones"]
            decay_per_step = optimizer_param["decay_per_step"]
            ...
            decay_per_step = optimizer_param.get("decay_per_step", 1.0)
    ...
    def train(self):
        ...
        if isinstance(train_dataset, BaseIterableDataset):
    ...
    def evaluation(self):
        ...
        # ====== Generate dataset ======
        test_dataset = self.dataset_class(dataset_param)
    ...
    def validation_step(self, model: nn.Module, data_loader: DataLoader, epoch: int, is_last_epoch: bool) -> Dict:
        ...
        with torch.no_grad():
            for _, val_data in enumerate(data_loader):
                ...
                val_inputs, val_labels, args = self.post_transform_val_data(val_data)  # noqa

                outputs = model(val_inputs, **args)
