...
class PoolingLayer(BaseModule):
    ...
    def __init__(self, config: Dict, *args, **kwargs) -> None:
        ...
        self._keep_dim = config.get("keep_dim", True)
...
class MeanAggregator(PoolingLayer):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Aggregated feature.
        """
        return torch.mean(x, dim=self._agg_dim, keepdim=self._keep_dim)
...
class SUMAggregator(PoolingLayer):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Aggregated feature.
        """
        return torch.sum(x, dim=self._agg_dim, keepdim=self._keep_dim)
