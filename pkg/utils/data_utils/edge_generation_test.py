...
from pkg.utils.other_utils.monitor import monitor_cpu_usage
...
def test_generate_distance_based_edges(
    ...
    """
    end = min(batch_size, num_nodes)
    indices = [idx for idx in range(0, end)]
    sorted_indices_ny = generate_distance_based_edges_ny(node_coords, indices, sections, nodes_per_section)
    print("test_generate_distance_based_edges: ", 0, sorted_indices_ny.shape)
...
def test_generate_distance_based_edges_numba(
    ...
    sorted_indices_nb = generate_distance_based_edges_nb(node_coords[0], sections_nb, nodes_per_section_nb)
    print(
